...
if($ThisUserSql = $mysqli->query("SELECT * FROM users WHERE id='$upid'")){
   ...
   $ThisUN = stripslashes($ThisUserRow['username']);
   		...
   		$ThisUserLong = stripslashes($ThisUserRow['about']);
		$ThisUserstrd = strlen ($ThisUserLong);
		if ($strd > 160) {
		$ThisAboutLong = stripslashes(substr($ThisUserLong,0,160)).'...';
		}else{
		$ThisAboutLong = stripslashes($ThisUserLong);}
		
		$ThisULink = preg_replace("![^a-z0-9]+!i", "-", $ThisUN);
		$ThisULink = strtolower($ThisULink);
...
<head>
<meta charset="utf-8">
<title><?php echo $ThisUN; ?>'s Profile | <?php echo $settings['name']; ?></title>
<meta name="description" content="<?php echo $ThisUserLong; ?>" />
...
<meta property="og:title"           content="<?php echo $ThisUN; ?>'s Profile | <?php echo $settings['name']; ?>" />
<meta property="og:description" 	content="<?php echo $ThisUserLong; ?>" /> 
<?php if (empty($avatar)){ ?>
<meta property="og:image"       	content="http://<?php echo $SiteLink;;?>/templates/<?php echo $settings['template'];?>/images/default-avatar.png" />
<?php }elseif (!empty($avatar)){?>
<meta property="og:image"       	content="http://<?php echo $SiteLink;;?>/avatars/<?php echo $ThisUserRow['avatar'];?>" />
<?php }?>
