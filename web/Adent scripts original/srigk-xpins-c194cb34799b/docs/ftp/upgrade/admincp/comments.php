...
<div class="page-header">
  <h3>Comments <small>Manage your website comments</small></h3>
</div>
...
// How many adjacent pages should be shown on each side?
	...
	//$query = $mysqli->query("SELECT COUNT(*) as num FROM comments LEFT JOIN posts ON photos.id=comments.photo_id WHERE photos.id=comments.photo_id ORDER BY comments.cmt_id DESC");
	...
	if($page) 
 	...
 	/* Get data. */
	$result = $mysqli->query("SELECT * FROM comments ORDER BY id DESC LIMIT $start, $limit");
	...
	$q= $mysqli->query("SELECT * FROM comments ORDER BY id DESC LIMIT $start,$limit");

	$numr = mysqli_num_rows($q);
	if ($numr==0)
	{
	echo '<div class="alert alert-danger" role="alert">There are no comments to display at this moment.</div>';
	}
	...
	$FullComment = stripslashes($Row['body']);
	$CropComment = strlen ($FullComment);
	if ($CropComment > 300) {
	$SortComment = substr($FullComment,0,300).'...';
	}else{
	$SortComment = $FullComment;}	
