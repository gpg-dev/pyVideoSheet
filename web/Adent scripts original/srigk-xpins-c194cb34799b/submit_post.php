...
{	
	...
	if(!isset($_FILES['inputfile']) && !count($_POST['image-link']))
	{
		//required variables are empty
	...
	$uploadFiles = [];
	$files = @$_FILES['inputfile'] ? $_FILES['inputfile'] : [];
	...
	if (!count($listNewName)) {
		$listUrl = $_POST['image-link'];
...
//	if(move_uploaded_file($_FILES['inputfile']["tmp_name"], $UploadDirectory . $NewImageName ))
	if(true)
    {
		$mysqli->query("INSERT INTO posts(image, images, url, embed, title, description, catid, uid, date, type, active) VALUES ('$NewImageName', '$imagesList','$PostLink','$EmbedCode','$PostTitle','$Description','$Catagory','$UserId','$AddedOn','$Type','$Active')");
?>
		<script>
...
<?php
		die('<div class="alert alert-success" style="display: block" role="alert">Your post submitted successfully.</div>');
	}else{
