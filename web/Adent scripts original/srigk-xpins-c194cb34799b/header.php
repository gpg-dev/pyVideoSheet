...
$listAd = [];
if($AdsSql = $mysqli->query("SELECT * FROM siteads WHERE type='list'")){
	$row = mysqli_fetch_array($AdsSql);
	$row['ad1'] && array_push($listAd, $row['ad1']);
	$row['ad2'] && array_push($listAd, $row['ad2']);
	$row['ad3'] && array_push($listAd, $row['ad3']);
	$AdsSql->close();
}
...
if($pageName == '/aboutus.html'){
	...
	$pageTitle = 'Videos | ';
} else if($pageName ==  '/audios.html'){
	$pageTitle = 'Audios | ';
} else if($pageName ==  '/your_posts.html'){
	$pageTitle = 'Your Post Gallery | ';
} else if($pageName ==  '/likes.html'){
	$pageTitle = 'your Likes Collection | ';
...
<head>
	...
	<style>
		...
		.dropdown.sub-level:hover .dropdown-menu {
			display: block;
		}
	</style>
