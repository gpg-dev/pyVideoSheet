	...
	while($row = mysqli_fetch_array($result))
	{
		...
		if ($Uid >0){
			...
			if($VcSql= $mysqli->query("SELECT uid FROM favip WHERE postid='$PostId' AND uid='$Uid'")){
				$query = $sqlGetProduct("uid=$upid", 'id');
				$result = $mysqli->query($query);
		...
		<div class="box">
			...
			<div class="play-button"><a href="<?php echo $link; ?>" target="_self"><span class="fa <?php echo $faIcon['video'] ?>"></span></a></div>
