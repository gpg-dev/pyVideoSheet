...
{	
	...
	if(!isset($_POST['to-user']) || strlen($_POST['to-user'])<1)
	{
		//required variables are empty
		die('<div class="alert alert-danger" role="alert">Please enter participant name</div>');
	}
	...
	$Username = $mysqli->escape_string($_POST['to-user']);

	if($ThisUserSql = $mysqli->query("SELECT * FROM users WHERE username='$Username'")){
