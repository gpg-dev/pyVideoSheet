...
if($_POST)
{
	...
	if(empty($_POST['inputCategory']) || strlen($_POST['inputCategory'])<1)
	{
	...
	{
		//required variables are empty
		die('<div class="alert alert-danger" role="alert">Please enter type of post</div>');
	}

	if((in_array($_POST['type'], [ $postType['video'] ]) && empty($_POST['embed'])) ) {
		//required variables are empty
		die('<div class="alert alert-danger" role="alert">Please enter embed url</div>');
	}
	if ( in_array($_POST['type'], [ $postType['video'] ]) ) {
		$embed = '<iframe width="725" height="410" src="'.$_POST['embed'].'" frameborder="0" allowfullscreen></iframe>';
	} else {
		$embed = '';
	}
	...
	if ($_POST['getTitle'] == 'urlTitle') {
		$PostTitle		= $mysqli->escape_string($_POST['urlTitle']); // file title
	}
	...
	*/
	...
	if(isset($_FILES['inputfile']))
	{
		...
		if(move_uploaded_file($_FILES['inputfile']["tmp_name"], $UploadDirectory . $NewImageName ))
		{
			...
			$mysqli->query("UPDATE posts SET image='$NewImageName',url='$PostLink',embed='$EmbedCode',title='$PostTitle',description='$Description',catid='$Catagory', type='$Type', active=1 WHERE id='$id'");
		...
		$updateImage = '';
		if ($_POST['getImage'] != 'user') {
			$url=$_POST['image-link'];
			...
			$updateImage = " image='$ImageFile',";
		} else {
			$ImageFile=$_POST['image-link'];
			$updateImage = " image='$ImageFile',";
		}
		$mysqli->query("UPDATE posts SET $updateImage url='$PostLink',embed='$EmbedCode',title='$PostTitle',description='$Description',catid='$Catagory', type='$Type', active=1 WHERE id='$id'");
