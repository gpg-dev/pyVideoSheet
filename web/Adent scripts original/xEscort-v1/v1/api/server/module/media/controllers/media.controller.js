...
const LIMIT_MEDIA_COUNT = 10; // every escort will have maximum 10 photos for now. TODO - move this to config
...
exports.validateLimitMedia = async (req, res, next) => {
  try {
    if (req.user.role === 'admin') {
      return next();
    }
    ...
    const count = await DB.Media.count({ 
      ownerId: req.user.role === 'admin' ? req.body.ownerId : req.user._id
    });
    if (count >= LIMIT_MEDIA_COUNT) {
      return next(PopulateResponse.error({
        message: `Maximum photo reached ${LIMIT_MEDIA_COUNT} items. Please delete one and reupload.`
      }, 'ERR_PHOTO_LIMIT'));
    }
